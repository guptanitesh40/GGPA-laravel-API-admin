$(document).ready(function() {
	...
	$('body').on('click','.changeSliderStatus',function() {
	...
	$('body').on('click','.deleteSlider',function() {
		...
		var current=$(this);
		var url=$('#delete_slider_url').val();
		var token=$('input[name=_token]').val();
		var id=current.attr('data');

	      swal({
	        title: "Are you sure ?",
	        text: "You will not be able to recover this slider details",
	        type: "warning",
			...
			$.ajax({
				...
				success:function(data) {
					if (data==1) {
						...
						swal("Deleted!", "Slider has been successfully deleted!", "success");
					}
					else {
						swal("Error", "An error occurred while deleting slider, Please try again later :)", "error");
					}
				}
			});
	      });
